/* 
 * grubefi.h - declare EFI types and functions 
 *
 * Note - copied from GRUB project (efi.h) and parts of types.h
 *        merged in.
 */
/*
...
#define GRUB_EFI_EXPANDED_ACPI_UIDSTR(dp)	\
  (GRUB_EFI_EXPANDED_ACPI_HIDSTR(dp) \
   + strlen (GRUB_EFI_EXPANDED_ACPI_HIDSTR(dp)) + 1)
#define GRUB_EFI_EXPANDED_ACPI_CIDSTR(dp)	\
  (GRUB_EFI_EXPANDED_ACPI_UIDSTR(dp) \
   + strlen (GRUB_EFI_EXPANDED_ACPI_UIDSTR(dp)) + 1)
